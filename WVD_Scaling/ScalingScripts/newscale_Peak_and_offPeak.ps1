...
try {
	...
	[string[]]$RequiredParams = @('SessionThresholdPerCPU', 'MinimumNumberOfRDSH', 'LimitSecondsToForceLogOffUser', "PeakMinRDSH")
	...
	[int]$PeakMinRDSH = $RqtParams.PeakMinRDSH
	...
	function Set-nVMsToStartOrStop {
		...
		if ($InPeakHours) {
			...
			if ($nRunningVMs -lt $PeakMinRDSH) {
				$res.nVMsToStart = ($PeakMinRDSH - $nRunningVMs)
				Write-Log "[In peak hours] Number of user sessions per Core is more than the threshold. Need to start $($res.nVMsToStart) cores"
			}
			elseif ($nUserSessionsPerCore -gt $UserSessionThresholdPerCore) {
        ...
        elseif ($nUserSessionsPerCore -gt $UserSessionThresholdPerCore) {
            $res.nCoresToStart = [math]::Ceiling(($nUserSessions / $UserSessionThresholdPerCore) - $nRunningCores)
            Write-Log "[Off peak hours] Number of user sessions per Core is more than the threshold. Need to start $($res.nCoresToStart) cores"
	}
